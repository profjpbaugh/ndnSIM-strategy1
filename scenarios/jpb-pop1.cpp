...
namespace ns3 {
  ...
  //for a node - the router (node 1)
     ...
     //-------------------------------------------------------------//
     //----> This can be shared by both PStrat and the Policy
     //----> Used to maintain and convey information between the two
     shared_ptr<PopularityManager> popularityManager = 
                                   make_shared<PopularityManager>(myContentStore.getLimit());

     cout<<"In jpb-pop1 main: "<<&popularityManager<<endl;
     ...
     nfd::fw::PStrat& effectiveStrategy = (nfd::fw::PStrat&)(sc.findEffectiveStrategy(nfd::Name("/prefix")));     
